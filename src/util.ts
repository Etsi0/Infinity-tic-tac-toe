 ...
 * }
 */
export function isMiddleOrCorner(position: coords): boolean {
 ...
 * }
 */
export function isMiddle(position: coords): boolean {
	const middle = Math.floor(settings.numCells / 2);
	return (
		(position.x === middle || (settings.numCells % 2 === 0 && position.x === middle - 1)) &&
 ...
 * }
 */
export function isCorner(position: coords): boolean {
	return (
		(position.x === 0 || position.x === settings.numCells - 1) &&
