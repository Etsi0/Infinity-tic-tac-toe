...
==================================================*/
export const squares = Array.from({ length: numCells }, (v, row) =>
	Array.from({ length: numCells }, (v, col) => {
		let priority = 0;
		if (
			(row === 0 && col === 0) ||
...
export function setSquares(position: number[], type: string, value: number) {
...
const preloaded: { [x: string]: HTMLImageElement } = Preload();
...
function GameLoop() {
	...
	const squareSize =
		(dimensions.boardSize - PercentageToPixels(lineThickness) * (numCells - 1)) / numCells;
	...
	ctx.fillText(
		`${turn === 'PLAYER' ? (oWon ? 'AI' : turn) : xWon ? 'PLAYER' : turn} turn`.toUpperCase(),
		canvas.width / 2,
		turnText
	);
	...
	if (oWon || xWon) {
		...
		ctx.textBaseline = 'middle';
		ctx.fillText(
			`${oWon ? 'O' : xWon ? 'X' : 'Who'} Won`,
			canvas.width / 2,
			canvas.height - turnText
		);
	}
...
/*==================================================
	DrawPieces
==================================================*/
function DrawPieces(n: number, piece: string, squareSize: number) {
	if (!history[piece][n]) {
		return;
	}
	let image: HTMLImageElement = preloaded[piece];
...
/*==================================================
	WinCon
==================================================*/
function WinCon(piece: string, currentValue: number[], direction: number[], count: number) {
	const checkArray = [
		[-1, -1],
		[0, -1],
		[1, -1],
...
function isMatchingPiece(piece: string, position: number[]): boolean {
	for (let i = 0; i < history[piece].length; i++) {
